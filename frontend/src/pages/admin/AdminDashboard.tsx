...
export default function AdminDashboard() {
  ...
  const handleApproveCustomRequest = async (id: number, price: number) => {
    try {
      ...
      toast({
        title: "Approved",
        description: "Request approved and quote set successfully.",
      });
      setCustomizations(
        customizations.map((c) => (c.id === id ? res.data.request : c)),
      );
    } catch (err: any) {
      toast({
        title: "Failed to approve",
        description: err.response?.data?.error || "Error setting quote.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeclineCustomRequest = async (id: number) => {
    ...
    try {
      const res = await api.put(`/customizations/${id}/status`, {
        status: "declined",
      });
      toast({
        title: "Declined",
        description: "Request status set to declined.",
      });
      setCustomizations(
        customizations.map((c) => (c.id === id ? res.data.request : c)),
      );
  ...
  const handleAddArtwork = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (artImageFile) {
        ...
        formData.append(
          "image_url",
          artImageUrl || "https://via.placeholder.com/600x400",
        );
      }
  ...
  const handleEditArtwork = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Artwork details updated successfully!",
      });
      ...
      toast({
        title: "Update failed",
        description:
          err.response?.data?.error || "Could not update artwork details.",
        variant: "destructive",
      });
    } finally {
      setArtSubmitting(false);
  ...
  const handleDeleteArtwork = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this artwork? This action is permanent.",
      )
    )
      return;
    try {
      ...
      toast({
        ...
        description: err.response?.data?.error || "Could not delete artwork.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteCoupon = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: err.response?.data?.error || "Could not delete coupon.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleToggleCouponActive = async (coupon: Coupon) => {
    try {
      ...
      toast({
        title: "Updated",
        description: `Coupon ${coupon.code} status updated.`,
      });
      fetchCoupons();
    } catch (err: any) {
      toast({
  ...
  const handleUpdateOrderStatus = async () => {
    ...
    try {
      ...
      setOrders(
        orders.map((o) => (o.id === selectedOrder.id ? finalUpdated : o)),
      );
      setSelectedOrder(finalUpdated);
    } catch (err: any) {
      toast({
        title: "Update failed",
        description: err.response?.data?.error || "Could not save updates.",
        variant: "destructive",
      });
    } finally {
  ...
  const activeOrdersList = orders.filter((o) =>
    ["placed", "confirmed", "processing", "shipped"].includes(o.order_status),
  );
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans">
      ...
      <header className="border-b bg-white px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-semibold text-lg text-slate-800">
            Artsy Pisces — Control Panel
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-500">
            Admin: <strong className="text-slate-800">{user?.name}</strong>
          </span>
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate("/")}
            className="gap-1.5 shadow-sm border-slate-200"
          >
      ...
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        <aside className="w-full md:w-64 bg-white border-r border-slate-100 p-5 space-y-2 flex-shrink-0">
          <p className="text-[10px] uppercase tracking-[0.2em] text-slate-400 font-semibold px-3 mb-4">
            Management
          </p>
        ...
        <main className="flex-1 p-8 overflow-y-auto">
            ...
            <div className="space-y-8">
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Quick business metrics and system status.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                ...
                <div className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm flex items-center gap-4">
                  <div
                    className={`p-4 rounded-xl ${netProfit >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                  >
                    <TrendingUp className="h-6 w-6" />
                  </div>
                  <div>
                    ...
                    <p
                      className={`text-2xl font-extrabold ${netProfit >= 0 ? "text-emerald-600" : "text-red-500"}`}
                    >
                      {netProfit >= 0 ? "₹" : "-₹"}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Add, edit details, or remove artwork listings.
                  </p>
                </div>
              ...
              {loadingArtworks ? (
                ...
                <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center text-slate-500 shadow-sm">
                  No artworks in gallery yet. Click "Add Artwork" to publish
                  one!
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {artworks.map((art) => (
                    <div
                      key={art.id}
                      className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm flex flex-col hover:shadow-md transition-shadow"
                    >
                      <div className="relative h-48 bg-slate-100 flex-shrink-0">
                        ...
                        />
                        <span
                          className={`absolute top-3 right-3 text-[10px] font-bold px-2 py-0.5 rounded-full border shadow-sm ${
                            art.available
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Monitor active sales and update dispatch tracking.
                  </p>
                </div>
                        ...
                        <tbody>
                          {activeOrdersList.map((order) => (
                            ...
                            >
                              ...
                              <td className="p-3">
                                <span
                                  className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold capitalize ${statusColor[order.order_status]}`}
                                >
                                  {order.order_status}
                                </span>
                              </td>
              ...
              <div>
                <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm sticky top-24 space-y-6">
                  <h3 className="font-semibold text-lg text-slate-800 border-b pb-3 flex items-center gap-2">
                    <Settings className="h-5 w-5 text-primary" /> Tracking
                    Management
                  </h3>
                  ...
                  {selectedOrder ? (
                    <div className="space-y-6 text-sm">
                      <div>
                        <h4 className="font-semibold text-slate-700 font-mono text-sm">
                          ORDER #
                      ...
                      {/* Items */}
                      <div className="space-y-1.5">
                        <h5 className="font-semibold text-slate-400 text-xs uppercase tracking-wider">
                          Purchased Items
                        </h5>
                      ...
                      <div className="border-t border-slate-100 pt-4 space-y-4">
                        ...
                        <Button
                          ...
                          className="w-full h-10 shadow-sm mt-1"
                        >
                          {orderSubmitting
                            ? "Updating..."
                    ...
                    <div className="text-center py-16 text-slate-400 text-sm flex flex-col items-center justify-center gap-2">
                      ...
                      <p>
                        Select a customer order from the table list to inspect
                        or modify its shipping state.
                      </p>
                    </div>
                  )}
                </div>
          ...
          {activeTab === "past_orders" && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-slate-800">
                    Archived Past Orders
                  </h2>
                  <p className="text-slate-500 text-sm">
                    History of delivered and cancelled customer orders.
                  </p>
                </div>
          ...
          {activeTab === "customizations" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold text-slate-800">
                  Custom Artwork Demands
                </h2>
                <p className="text-slate-500 text-sm">
                  Review user ideas, check reference images, and submit custom
                  pricing quotes.
                </p>
              </div>
              ...
              {loadingCustomizations ? (
                <div className="space-y-4 py-4">
                  {[1, 2].map((i) => (
                    <div
                      key={i}
                      className="h-24 bg-white rounded-2xl animate-pulse border"
                    />
                  ))}
                </div>
                  ...
                  <table className="w-full text-left border-collapse text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        <th className="p-4 font-semibold text-right text-slate-600">
                          Actions
                        </th>
                      </tr>
                    </thead>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Add active coupon discount policies and specify usage
                    limitations.
                  </p>
                </div>
                <Button
              ...
              {loadingCoupons ? (
                <div className="space-y-4 py-4">
                  {[1, 2].map((i) => (
                    <div
                      key={i}
                      className="h-16 rounded-xl bg-white border border-slate-100 animate-pulse"
                    />
                  ))}
                </div>
              ) : coupons.length === 0 ? (
                <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center text-slate-500 shadow-sm">
                  No active coupon campaigns yet. Click "Create Coupon" to
                  launch one!
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse text-sm">
                    ...
                    <tbody>
                      {coupons.map((coupon) => (
                        <tr
                          key={coupon.id}
                          className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/20"
                        >
          ...
          {activeTab === "settings" && (
            <div className="max-w-md space-y-6">
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Change your admin password. Your email cannot be changed.
                </p>
              </div>
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  This email is permanently set and cannot be changed.
                </p>
              </div>
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-5">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="cp-current" className="text-sm">
                    Current Password
                  </Label>
                  <Input
                    id="cp-current"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="cp-new" className="text-sm">
                    New Password
                  </Label>
                  <Input
                    id="cp-new"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="cp-confirm" className="text-sm">
        ...
        <div className="fixed inset-0 bg-black/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border rounded-2xl w-full max-w-lg shadow-2xl p-6 relative flex flex-col max-h-[90vh]">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="Abstract, Traditional..."
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="add-quote">
                  Quote (Displayed in gallery hover)
                </Label>
                <Input
                  id="add-quote"
                  value={artQuote}
                  onChange={(e) => setArtQuote(e.target.value)}
                  placeholder="e.g. 'A soft metallic whisper.'"
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 gap-2 border-slate-200">
                <Label className="text-slate-600 font-semibold cursor-pointer text-center w-full">
                  ...
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      setArtImageFile(e.target.files?.[0] || null)
                    }
                    className="hidden"
                  />
                </Label>
                {artImageFile ? (
                  <p className="text-xs text-primary font-bold">
                    Selected: {artImageFile.name}
                  </p>
                ) : (
                  <div className="w-full text-center space-y-2">
                    <span className="text-[10px] text-slate-400">
                      or provide image URL below
                    </span>
                    <Input
                      value={artImageUrl}
          ...
          <div className="bg-white border rounded-2xl w-full max-w-lg shadow-2xl p-6 relative flex flex-col max-h-[90vh]">
            ...
            >
              ...
              <div className="border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 gap-2 border-slate-200">
                ...
                {artImageFile ? (
                  <p className="text-xs text-primary font-bold">
                    Replacement: {artImageFile.name}
                  </p>
                ) : (
                  <p className="text-[10px] text-slate-400">
                    Keep blank to preserve existing image URL
                  </p>
                )}
              </div>
              ...
              >
                {artSubmitting
                  ? "Updating and uploading replacement..."
                  : "Save Changes"}
              </Button>
            </form>
          </div>
      ...
      {showAddCouponModal && (
        <div className="fixed inset-0 bg-black/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border rounded-2xl w-full max-w-md shadow-2xl p-6 relative flex flex-col">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-1.5">
              <PlusCircle className="h-5 w-5 text-primary" /> Create Promo
              Coupon
            </h3>
  ...
  return (
    <tr className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/20">
      ...
      <td className="p-4 max-w-xs">
        <p className="text-slate-700 whitespace-pre-wrap leading-relaxed text-xs line-clamp-3 hover:line-clamp-none transition-all">
          {request.description}
        </p>
      </td>
      <td className="p-4">
        {request.reference_image_url ? (
          ...
          >
            <img
              ...
              alt="Ref"
              className="w-12 h-12 object-cover rounded border group-hover:scale-110 transition-transform"
            />
          </a>
        ) : (
      ...
      <td className="p-4">
        <span
          className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold border capitalize ${
